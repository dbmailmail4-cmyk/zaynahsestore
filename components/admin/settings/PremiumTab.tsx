...
interface PremiumTabProps {
  ...
  setSocialFeedsHomepageEnabled: (v: boolean) => void;
  socialFeedsProductEnabled: boolean;
  setSocialFeedsProductEnabled: (v: boolean) => void;
  socialFeedsTitle: string;
  setSocialFeedsTitle: (v: string) => void;
  socialFeedsSubtitle: string;
  setSocialFeedsSubtitle: (v: string) => void;
  socialFeedsDesc: string;
  setSocialFeedsDesc: (v: string) => void;
  socialFeedsItems: any[];
  setSocialFeedsItems: React.Dispatch<React.SetStateAction<any[]>>;
  tempFeedUsername: string;
  setTempFeedUsername: (v: string) => void;
  tempFeedLink: string;
  setTempFeedLink: (v: string) => void;
  tempFeedImageUrl: string;
  setTempFeedImageUrl: (v: string) => void;
  tempFeedCaption: string;
  setTempFeedCaption: (v: string) => void;
...
export default function PremiumTab({
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
      ...
      {/* Social Feeds Config Card */}
      <div className="bg-white dark:bg-[#16162a] border border-gray-200 dark:border-gray-800 rounded-2xl p-6 shadow-sm space-y-4 col-span-1 md:col-span-2">
        <h3 className="text-sm font-extrabold text-[#e94560] uppercase tracking-wider">Social Feeds Ribbon Customizer</h3>
        <p className="text-xs text-gray-500 dark:text-gray-400">Configure username, title, and social images feed display on storefront.</p>
        
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <label className="flex items-center gap-3 p-3 rounded-xl border border-gray-100 dark:border-gray-850 bg-gray-50/50 dark:bg-white/5 cursor-pointer select-none">
            <input
              type="checkbox"
              checked={socialFeedsHomepageEnabled}
              onChange={(e) => setSocialFeedsHomepageEnabled(e.target.checked)}
              className="rounded border-gray-300 text-[#e94560] focus:ring-[#e94560] h-4 w-4"
            />
            <span className="text-xs font-bold text-gray-700 dark:text-gray-300">Show on Homepage storefront</span>
          </label>
          
          <label className="flex items-center gap-3 p-3 rounded-xl border border-gray-100 dark:border-gray-850 bg-gray-50/50 dark:bg-white/5 cursor-pointer select-none">
            <input
              type="checkbox"
              checked={socialFeedsProductEnabled}
              onChange={(e) => setSocialFeedsProductEnabled(e.target.checked)}
              className="rounded border-gray-300 text-[#e94560] focus:ring-[#e94560] h-4 w-4"
            />
            <span className="text-xs font-bold text-gray-700 dark:text-gray-300">Show on Product details page</span>
          </label>
        </div>

        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div className="space-y-2">
            <label className="text-xs font-bold text-gray-700 dark:text-gray-300 block">Ribbon Title</label>
            <input
              type="text"
              value={socialFeedsTitle}
              onChange={(e) => setSocialFeedsTitle(e.target.value)}
              className="w-full px-3 py-2 text-xs rounded-xl border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-[#0f0f1b] text-gray-955 dark:text-gray-50 focus:outline-none focus:ring-2 focus:ring-[#e94560]"
            />
          </div>
          <div className="space-y-2">
            <label className="text-xs font-bold text-gray-700 dark:text-gray-300 block">Ribbon Subtitle (Username)</label>
            <input
              type="text"
              value={socialFeedsSubtitle}
              onChange={(e) => setSocialFeedsSubtitle(e.target.value)}
              className="w-full px-3 py-2 text-xs rounded-xl border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-[#0f0f1b] text-gray-955 dark:text-gray-50 focus:outline-none focus:ring-2 focus:ring-[#e94560]"
            />
          </div>
          <div className="space-y-2 col-span-1 sm:col-span-2">
            <label className="text-xs font-bold text-gray-700 dark:text-gray-300 block">Description / Tagline text</label>
            <input
              type="text"
              value={socialFeedsDesc}
              onChange={(e) => setSocialFeedsDesc(e.target.value)}
              className="w-full px-3 py-2 text-xs rounded-xl border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-[#0f0f1b] text-gray-955 dark:text-gray-50 focus:outline-none focus:ring-2 focus:ring-[#e94560]"
            />
          </div>
        </div>

        {/* Social feed items array list manager */}
        <div className="border border-gray-200 dark:border-gray-800 rounded-xl p-4 bg-gray-50 dark:bg-[#0f0f1b] space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="space-y-2">
              <label className="text-[10px] font-bold text-gray-500 dark:text-gray-400 uppercase">Step 1: Upload Post Image</label>
                ...
                {tempFeedImageUrl ? (
                  <div className="relative w-16 h-16 rounded-lg overflow-hidden border border-gray-200 dark:border-gray-700">
                    ...
                    <button
                      ...
                      className="absolute inset-0 bg-black/55 flex items-center justify-center text-white opacity-0 hover:opacity-100 transition-opacity"
                    >
            ...
            <div className="space-y-3">
              <div className="space-y-1">
                ...
                <input
                  ...
                  className="w-full px-3 py-1.5 text-xs rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-[#16162a] text-gray-955 dark:text-gray-50 focus:outline-none"
                />
              </div>
              <div className="space-y-1">
              ...
              <div className="space-y-1">
                ...
                <input
                  type="text"
                  placeholder="e.g. Beautiful summer wear set..."
          ...
          <div className="pt-2">
            <label className="text-[10px] font-bold text-gray-500 dark:text-gray-400 uppercase block mb-2">Feed Preview ({socialFeedsItems.length} items)</label>
            <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 gap-3">
              {socialFeedsItems.map((item, idx) => (
                <div key={item.id || idx} className="relative group aspect-square rounded-lg overflow-hidden border border-gray-200 dark:border-gray-800">
                  {/* eslint-disable-next-line @next/next/no-img-element */}
                  <img src={item.imageUrl} alt="Social post" className="w-full h-full object-cover" />
                  <div className="absolute inset-0 bg-black/75 flex flex-col justify-between p-1.5 opacity-0 group-hover:opacity-100 transition-opacity duration-150">
                    <button
                      type="button"
                      onClick={() => handleDeleteSocialFeedItem(item.id || item.imageUrl)}
                      className="self-end text-red-500 hover:text-red-400 p-1 bg-white/10 rounded-md"
                    >
                      <Trash2 className="h-3 w-3" />
                    </button>
                    <div className="min-w-0">
                      <p className="text-[9px] font-bold text-white truncate">@{item.username}</p>
                      {item.caption && <p className="text-[8px] text-gray-300 truncate">{item.caption}</p>}
                    </div>
                  </div>
                </div>
              ))}
              {socialFeedsItems.length === 0 && (
                <div className="col-span-full text-center py-6 text-xs text-gray-400">
                  No feed posts added. Add images to construct an Instagram feed ribbon.
                </div>
              )}
            </div>
          </div>
