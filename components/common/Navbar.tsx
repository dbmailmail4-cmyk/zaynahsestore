...
import { ShoppingCart, LayoutDashboard, Search, X, Heart, ChevronDown, User } from '@/components/common/Icons';
        ...
        {navigationMenu.map((item) => {
          ...
          return (
            ...
            >
              ...
              {/* Dropdown sub-menu */}
              {hasChildren && isOpen && (
                <div className="absolute top-full left-0 mt-1 min-w-[180px] bg-white dark:bg-[#16162a] border border-gray-200 dark:border-gray-700 rounded-2xl shadow-xl py-2 z-50 animate-fade-in">
                  {item.children!.map((child) => (
                    <Link
                      key={child.id}
                      href={child.url}
                      className="block px-4 py-2 text-sm font-semibold text-gray-700 dark:text-gray-200 hover:text-[#e94560] hover:bg-gray-50 dark:hover:bg-white/5 transition-colors"
                    >
                      {child.label}
                    </Link>
                  ))}
    ...
    <>
      ...
      {mounted && mobileMenuOpen && (
        <div
          ...
          className="fixed inset-0 z-50 bg-black/60 backdrop-blur-md flex justify-start transition-all duration-300 animate-fade-in"
        >
          <div
                      ...
                      return (
                        <div key={item.id}>
                          ...
                          {hasChildren && isAccOpen && (
                            <div className="ml-4 mt-1 space-y-1 border-l-2 border-[#e94560]/20 pl-3 animate-fade-in">
                              {item.children!.map((child) => (
                                <Link
                                  key={child.id}
                                  href={child.url}
                                  onClick={() => setMobileMenuOpen(false)}
                                  className="block py-1.5 text-sm font-semibold text-gray-600 dark:text-gray-300 hover:text-[#e94560] transition-colors"
                                >
                                  {child.label}
                                </Link>
                              ))}
      ...
      {mounted && searchOpen && !isAdmin && (
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-md flex items-start justify-center pt-24 px-4 transition-all duration-300 animate-fade-in overscroll-contain">
          <div ref={containerRef} className="bg-white dark:bg-[#16162a] w-full max-w-2xl rounded-3xl border border-gray-200 dark:border-gray-800 shadow-2xl p-6 relative scale-up transition-all">
            ...
            {searchQuery.trim().length === 0 && (
              <div className="space-y-3 pt-2">
                ...
                <div className="flex flex-wrap gap-2">
                  {['Co-ord Sets', 'Sonic', 'Graphic Tee', 'T-shirt', 'Kids'].map((term) => (
                    <button
                      key={term}
                      onClick={() => {
                        setSearchQuery(term);
